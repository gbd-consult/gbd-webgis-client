...
class TLayerWms extends TLayerGroup {
    async init(cnf) {
        ...
        if (!cnf.layers) {
            let capsDoc = await wms.capabilities.load(cnf.url, cnf.params);
            this.caps = capsDoc.layers[0];
            this.initFromCaps(false);
        } else {
            // @TODO
            // this.caps = this.config.caps;
            // this.config.shouldRender = true;
            // this.initFromLayers(cnf.layers)
        }
        if (cnf.title)
    ...
    initFromCaps(hasComposer) {
        ...
        if (hasComposer) {
            ...
            this.config.shouldRender = hasComposer || !ls.length;
        }
    ...
    createMapLayer() {
        ...
        let source = new ol.source.ImageWMS({
            ...
            params: {
                ...this.config.params,
                layers: this.isGroup() ? [] : [this.caps.name]
            },
            ratio: 1,
            preload: 0,
        });
    ...
    async wmsQuery(coordinate) {
        if (!this.wmsRoot || !this.isVisible())
            return [];

        let leaves = this.visibleLeaves().reverse();
